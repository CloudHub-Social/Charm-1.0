...
/**
 * Namespaces every capture by `testInfo.project.name` (`chromium` / `mobile`)
 * so the same spec running under both playwright.config.ts projects doesn't
 * have one project's screenshot silently overwrite the other's at the same
 * path.
 */
