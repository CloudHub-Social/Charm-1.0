import { Avatar, AvatarFallback, AvatarImage, Box, Text, Icon, Icons } from 'folds';

export function Sizes() {
  return (
    <Box direction="Row" wrap="Wrap" gap="300" alignItems="Center">
      <Avatar size="500">
        <AvatarFallback style={{ backgroundColor: '#6e56cf', color: 'white' }}>
          <Text size="H3">EG</Text>
        </AvatarFallback>
      </Avatar>
      <Avatar size="400">
        <AvatarFallback style={{ backgroundColor: '#6e56cf', color: 'white' }}>
          <Text size="H4">EG</Text>
        </AvatarFallback>
      </Avatar>
      <Avatar size="300">
        <AvatarFallback style={{ backgroundColor: '#6e56cf', color: 'white' }}>
          <Text size="H5">EG</Text>
        </AvatarFallback>
      </Avatar>
      <Avatar size="200">
        <AvatarFallback style={{ backgroundColor: '#6e56cf', color: 'white' }}>
          <Text size="H6">EG</Text>
        </AvatarFallback>
      </Avatar>
    </Box>
  );
}

export function Variants() {
  return (
    <Box direction="Row" wrap="Wrap" gap="300" alignItems="Center">
      <Avatar size="400">
        <AvatarImage src="https://i.pravatar.cc/150?img=12" alt="@ada:charm.im" />
      </Avatar>
      <Avatar size="400">
        <AvatarFallback style={{ backgroundColor: '#0ea5e9', color: 'white' }}>
          <Text size="H4">AB</Text>
        </AvatarFallback>
      </Avatar>
      <Avatar size="400">
        <AvatarFallback style={{ backgroundColor: '#e5484d', color: 'white' }}>
          <Icon size="400" src={Icons.Hash} />
        </AvatarFallback>
      </Avatar>
    </Box>
  );
}

export function Radii() {
  return (
    <Box direction="Row" wrap="Wrap" gap="300" alignItems="Center">
      <Avatar size="400" radii="Pill">
        <AvatarFallback style={{ backgroundColor: '#6e56cf', color: 'white' }}>
          <Text size="H4">RT</Text>
        </AvatarFallback>
      </Avatar>
      <Avatar size="400" radii="400">
        <AvatarFallback style={{ backgroundColor: '#12a594', color: 'white' }}>
          <Text size="H4">RT</Text>
        </AvatarFallback>
      </Avatar>
      <Avatar size="400" radii="0">
        <AvatarFallback style={{ backgroundColor: '#f76808', color: 'white' }}>
          <Text size="H4">RT</Text>
        </AvatarFallback>
      </Avatar>
    </Box>
  );
}
