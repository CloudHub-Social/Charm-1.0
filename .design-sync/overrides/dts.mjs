// forked from design-sync lib/dts.mjs — folds ships .d.ts in dist/ but declares no `types` field, so the package.json-derived entry (pkgDir/index.d.ts) doesn't exist; fall back to typesRoot/index.d.ts
...
function projectFor(pkgDir, typesRoot) {
  ...
  // FORK: packages that ship .d.ts under dist/ without declaring `types`
  // (e.g. folds) resolve entry to a non-existent pkgDir/index.d.ts. Fall back
  // to the discovered typesRoot's index.d.ts so getSourceFile(entry) matches.
  if (!existsSync(entry) && typesRoot && existsSync(join(typesRoot, 'index.d.ts'))) {
    entry = join(typesRoot, 'index.d.ts');
  }
