...
const previewRequestCache = new WeakMap<
  MatrixClient,
  Map<string, Promise<IPreviewUrlResponse>>
>();
...
const previewResultCache = new WeakMap<
  MatrixClient,
  Map<string, PreviewResultEntry>
>();
...
function isLikelyPlayableOgVideo(prev: IPreviewUrlResponse): boolean {
  ...
  if (!url) return false;
  const mime =
    typeof prev["og:video:type"] === "string"
      ? prev["og:video:type"].toLowerCase().trim()
      : "";
  if (mime.startsWith("video/")) return true;
  if (/^mxc:\/\//i.test(url)) {
    return (
      mime.startsWith("video/") || /\.(mp4|webm|ogg|mov|m4v)(\?|$)/i.test(url)
    );
  }
  if (/^https?:\/\//i.test(url)) {
    return (
      /\.(mp4|webm|ogg|mov|m4v)(\?|$)/i.test(url) || mime.startsWith("video/")
    );
  }
  return false;
}
...
>(
  (
    { urlPreview, url, ts, mediaType, mediaAutoLoad = true, bundle, ...props },
    ref,
  ) => {
    const mx = useMatrixClient();
    const useAuthentication = useMediaAuthentication();
    const [linkPreviewImageMaxHeight] = useSetting(
      settingsAtom,
    ...
    const previewVideoUrl = useMemo(() => {
      ...
      if (videoUrl.startsWith("mxc://"))
        return mxcUrlToHttp(mx, videoUrl, useAuthentication);
      return videoUrl.startsWith("http") ? videoUrl : undefined;
    }, [mx, previewData, useAuthentication]);
    const previewImageMetadata = useMediaMetadata(
      previewImageMetadataUrl
        ? getScopedMediaCacheKey(previewImageMetadataUrl)
        : undefined,
    );
    const previewVideoMetadata = useMediaMetadata(
      previewVideoUrl ? getScopedMediaCacheKey(previewVideoUrl) : undefined,
    );
    const directMediaMetadata = useMediaMetadata(
      isDirect ? getScopedMediaCacheKey(url) : undefined,
    );
    ...
    const renderDirectMedia = () => {
      ...
      if (directAnimatedPreviewRef.current?.url !== url) {
        directAnimatedPreviewRef.current = {
          url,
          isAnimated: isAnimatedDirectImage(
            body,
            directMediaMetadata?.mimeType,
          ),
        };
      }
    ...
    const renderContent = (prev: IPreviewUrlResponse) => {
      ...
      const videoW = prev["og:video"]
        ? ogPositiveDimension(prev["og:video:width"])
        : undefined;
      const videoH = prev["og:video"]
        ? ogPositiveDimension(prev["og:video:height"])
        : undefined;
      const ogImgW = ogPositiveDimension(prev["og:image:width"]);
      const ogImgH = ogPositiveDimension(prev["og:image:height"]);
      const cachedVideoW = prev["og:video"]
        ? previewVideoMetadata?.width
        : undefined;
      const cachedVideoH = prev["og:video"]
        ? previewVideoMetadata?.height
        : undefined;
      const cachedImgW = prev["og:image"]
        ? previewImageMetadata?.width
        : undefined;
      const cachedImgH = prev["og:image"]
        ? previewImageMetadata?.height
        : undefined;
      ...
      const previewBlurRaw =
        typeof prev["matrix:image:blurhash"] === "string"
          ? prev["matrix:image:blurhash"].trim()
          : "";

      const ogImageInfo: IImageInfo | undefined = (() => {
        const matrixSize = prev["matrix:image:size"];
        const size =
          typeof matrixSize === "number" && Number.isFinite(matrixSize)
            ? matrixSize
            : undefined;
        ...
        if (resolvedImageW && resolvedImageH) {
          return {
            ...
            h: resolvedImageH,
            ...(size !== undefined ? { size } : {}),
            ...(previewImageMetadata?.mimeType
              ? { mimetype: previewImageMetadata.mimeType }
              : {}),
            ...(previewBlurRaw
              ? { [MATRIX_UNSTABLE_BLUR_HASH_PROPERTY_NAME]: previewBlurRaw }
              : {}),
          };
        }
      ...
      >
        <Box shrink="No" alignItems="Center">
          {canScrollLeft && (
            <>
              <div
                className={css.UrlPreviewHolderGradient({ position: "Left" })}
              />
              <IconButton
