...
export function useNotificationDeviceScope(
  ...
  const publishLease = useCallback(
    (nextNow: number, force = false): void => {
      if (!mx || !canPublishLease || !deviceId || typeof mx.setAccountData !== 'function') {
        return;
      }
  ...
  }, [canPublishLease, deviceId, mx, publishLease, shouldPublishLease, shouldHoldLease]);
  ...
  useEffect(() => {
    ...
    if (!desktopDelayEnabled || !canPublishLease || leaseRef.current?.deviceId !== deviceId) return;

    const nextNow = Date.now();
    setNow(nextNow);
    publishLease(nextNow, true);
  }, [canPublishLease, desktopDelayEnabled, deviceId, publishLease, shouldHoldLease]);
