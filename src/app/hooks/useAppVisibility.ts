...
export function useAppVisibility(mx: MatrixClient | undefined, activeSession?: Session) {
  ...
  const phase1ForegroundResync = sessionSyncConfig?.phase1ForegroundResync === true;
  const phase2VisibleHeartbeat = sessionSyncConfig?.phase2VisibleHeartbeat === true;
  const phase3AdaptiveBackoffJitter = sessionSyncConfig?.phase3AdaptiveBackoffJitter === true;
