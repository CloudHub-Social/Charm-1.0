...
function MessageNotifications() {
  ...
  const isActiveNotificationClientRef = useRef(isActiveNotificationClient);
  isActiveNotificationClientRef.current = isActiveNotificationClient;
  const leaseExpiresAtRef = useRef<number | null>(lease?.expiresAt ?? null);
  leaseExpiresAtRef.current = lease?.expiresAt ?? null;
  ...
  useEffect(() => {
    ...
    const handleTimelineEventImpl = (
      ...
      room: Parameters<RoomEventHandlerMap[RoomEvent.Timeline]>[1],
      _toStartOfTimeline: Parameters<RoomEventHandlerMap[RoomEvent.Timeline]>[2],
      _removed: Parameters<RoomEventHandlerMap[RoomEvent.Timeline]>[3],
      data: Parameters<RoomEventHandlerMap[RoomEvent.Timeline]>[4],
      forceActiveNotificationClient = false
    ) => {
    ...
    const handleTimelineEvent: RoomEventHandlerMap[RoomEvent.Timeline] = (
      ...
      removed,
      data
    ) => handleTimelineEventImpl(mEvent, room, toStartOfTimeline, removed, data);
