...
export const classifyCryptoStoreIndexedDbError = (
  ...
  if (errorMessage.includes("Transaction aborted"))
    return "transaction_aborted";
  if (errorMessage.includes("without an in-progress transaction"))
    return "transaction_error";
  if (errorMessage.includes("database connection is closing"))
    return "connection_closing";
  if (errorMessage.includes("database connection is closed"))
...
export const isCryptoStoreRuntimeRecoveryError = (
  ...
  const hasCryptoContext =
    options?.hasCryptoContext === true ||
    hasExplicitCryptoStoreContext(errorMessage);
  ...
  if (errorType === "invalid_state" || errorType === "unknown_idb_error") {
    return hasCryptoContext;
  }
